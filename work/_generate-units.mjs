...
const UNITS = [
  ...
  { id: "account-connect", title: "Account connect", entry: "modal from drawer", path: "src/screens/AccountScreen.tsx", container: "App.tsx", priority: "P2", status: "✅ done",
    pieces: { UI: ["ProfileDrawer header"], lib: ["account (Google Sign-In wrapper, silent restore, shared state)"] }, states: ["signed out", "signed in", "unconfigured build", "cancelled", "no Play services"], notes: ["Basic profile scope only; Drive comes with the sync unit via incremental auth", "Web client id is committed: a public identifier, the Android client binds sign-in to package + signing key", "Consent screen is in production so sessions do not expire weekly"] },
  ...
  { id: "drive-backup", title: "Drive backup", entry: "account screen", path: "src/lib/drive.ts", container: "AccountScreen", priority: "P1", status: "✅ done",
    pieces: { UI: ["AccountScreen backup/restore actions", "AccountAvatar"], lib: ["drive (folder, list, resumable upload, restore)", "account (drive.file scope, access token)"] }, states: ["backing up", "restoring", "nothing to do", "access refused", "offline", "no backup found"], notes: ["drive.file scope, asked on first backup only", "Files keep their 'Title [videoId].ext' name so the scanner recognises them after a restore", "Restore republishes each missing file through MediaSaver, the same path as a fresh download"] },
